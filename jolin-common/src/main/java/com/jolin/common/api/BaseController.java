...
public abstract class BaseController<S extends IBaseService, DTO extends BaseDTO> extends CommonController<S, DTO> {
    ...
    @ApiOperation(value = "2.BATCH_ADD")
    ...
    @ApiOperation(value = "4.DELETE_BY_ID")
    @ApiOperationSupport(order = 4)
    @DeleteMapping("/{id}")
    public ResultDTO<Boolean> deleteById(@PathVariable("id") @NotBlank Integer id) throws BaseException {
        Boolean isSuccess = iBaseService.removeById(id);
        return new ResultDTO<>(isSuccess);
    }

    @ApiOperation(value = "5.DELETE_BY_IDS")
    ...
    @ApiOperation(value = "6.GET_BY_ID")
